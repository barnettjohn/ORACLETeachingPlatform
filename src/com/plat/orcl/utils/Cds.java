...
public class Cds implements Serializable{
	...
	public void initCds(ResultSet rs) throws Exception{
		try{
            ...
            for (int i=0;i<_arrColumnName.length;i++){
              if (PubUtil.isEmptyString(_arrColumnName[i])){
	...
	public void initCds(String[] strColumnName,int[] intColumnType,int[] intColumnLength,int[] intColumnScale,HashMap hmFieldName,ArrayList arrayData) throws Exception{
	...
	//返回strFieldName字段＝strFieldValue值的所有记录序号，保持原记录顺序
	...
	private void checkFilterValid(FilterClause[] filterClause) throws Exception{
		...
		for (int i=0;i<filterClause.length;i++){
			...
			if (iOperator != PubConst.COMP_EQUAL){
				...
				if (objFilterValue instanceof String){
					if (PubUtil.isEmptyString(objFilterValue)){
						continue;
					}
				}else{
					String[] strValue=(String[])objFilterValue;
					if (strValue.length <= 0 || (strValue.length == 1 && PubUtil.isEmptyString(strValue[0]))){
						continue;
					}
				}
		...
		for (int i=0;i<a.size();i++){
			...
			arrOtherFilerDataType[i]=PubUtil.sql2JavaType(this._arrColumnType[iCol]);
	...
	private static boolean compareOtherValue_Between(int iDataType,Object objSrcValue,int iOperator,Object objFilterValue1,Object objFilterValue2){
	    if (iDataType == PubConst.DATATYPE_CHAR){
    		if (PubUtil.isEmptyString(objSrcValue)){
    			return false;
    		}else {
	...
	protected int compareOperator(String strOperator) throws Exception{
	  ...
	  protected int chineseCompare(String s1, String s2) 
	  {
		  ...
		  try{
			  ...
			  for (int i = 0; i < n; i ++)
			  {
				  ...
				  if (s1_code == s2_code) continue;
				  
				  if (s1_code > 0 && s2_code < 0) return -1;
				  if (s2_code > 0 && s1_code < 0) return 1;
	  ...
	  {
		  if (PubUtil._hmGBKChar.containsKey(c)){
			  return ((Integer)PubUtil._hmGBKChar.get(c)).intValue();
		  }

		  byte[] b = (new String(c.toString().getBytes(PubUtil.getCharSet()),"GBK")).getBytes();
		  int value = 0;
		  ...
		  PubUtil._hmGBKChar.put(c, new Integer(value));
	...
	private HashSet procOneEqualFilter(FilterClause oneFilterClause) throws Exception{
		...
		if (null == objValue){   //空筛选条件值表示不参与筛选
			...
			if (PubUtil.isEmptyString(objValue)){
				return null;
			}else{
	...
	public Cds cloneCds(){
		...
		for (int i=0;i<this._arrData.size();i++){
			...
			ArrayList aRowNew=PubUtil.cloneArrayList(aRow);
	...
	public Cds cloneCds_NoData(){
		Cds objRet=new Cds();
		objRet._arrColumnName=PubUtil.cloneString(this._arrColumnName);
		objRet._arrColumnType=PubUtil.cloneInt(this._arrColumnType);
		objRet._arrColumnLength=PubUtil.cloneInt(this._arrColumnLength);
		objRet._arrColumnScale=PubUtil.cloneInt(this._arrColumnScale);
		objRet._hmFieldName=PubUtil.cloneHashMap(this._hmFieldName);
	...
	public void appendField(String strColumnName,int intColumnType,int intColumnLength,int intColumnScale,Object objDefaultValue) throws Exception{
		...
		_arrColumnName=PubUtil.appendStringArray(_arrColumnName, strColumnName);
		_arrColumnType=PubUtil.appendIntArray(_arrColumnType, intColumnType);
		_arrColumnLength=PubUtil.appendIntArray(_arrColumnLength, intColumnLength);
		_arrColumnScale=PubUtil.appendIntArray(_arrColumnScale, intColumnScale);
	...
	public ArrayList getOneRow(int iRow){
		return (ArrayList)_arrData.get(iRow);
	}
	...
	public int appendRow() throws Exception{
		BigDecimal dZero=new java.math.BigDecimal(0);
		String strDef="";
	...
	public int getIntValue(ArrayList oneRow,String strFieldName){
		...
		if (PubUtil.isEmptyString(objValue)){//modify by sunkui 2014.4.30 增加objValue为空字符串的判断
			return 0;
		}else if (objValue instanceof BigDecimal){
  ...
  public boolean getBoolValue(int row, String fieldName) {
	...
	private Object getColumnValue(ResultSet rs,int intColumn,int intFieldType) throws Exception{
		...
		switch (intFieldType)
		{
			...
			case java.sql.Types.SMALLINT:
			case java.sql.Types.TINYINT:
				java.math.BigDecimal d1=rs.getBigDecimal(intColumn);
				if (null == d1)
			...
			case java.sql.Types.TIMESTAMP:
				...
				if (t != null){
					...
					//月
					String strMonth="00"+(1+t.getMonth());  strBufTime.append("-").append(strMonth.substring(strMonth.length()-2));
					//日
					String strDay="00"+t.getDate();   strBufTime.append("-").append(strDay.substring(strDay.length()-2));
					//时
					String strHour="00"+t.getHours();  strBufTime.append(" ").append(strHour.substring(strHour.length()-2)).append(":");
					//分
					String strMinus="00"+t.getMinutes();  strBufTime.append(strMinus.substring(strMinus.length()-2)).append(":");
					//秒
					String strSecond="00"+t.getSeconds();  strBufTime.append(strSecond.substring(strSecond.length()-2));
			...
			case java.sql.Types.NUMERIC:
			case java.sql.Types.REAL:
				java.math.BigDecimal d2=rs.getBigDecimal(intColumn);
				if (null == d2)
					return new java.math.BigDecimal(0);
				else
					return d2;
			...
			default: //字符数据
				s=PubUtil.nullToEmpty(rs.getString(intColumn));
				if (s == null)
					return "";
				else
					return s;
		...
		for (int iCol=0;iCol<this._arrColumnName.length;iCol++){
			if (this.isCharType(this._arrColumnType[iCol]) && this._arrColumnLength[iCol] >= 1800){
				for (int iRow=0;iRow<_arrData.size();iRow++){
					...
					if (false == PubUtil.isEmptyString(value) && value.length() > 900){
						return true;
					}
				}
			}
	...
	public void filterDupData(String[] arrDupField) throws Exception{
		...
		for (int i=0;i<arrDupField.length;i++){
			...
			iKeyDataType[i]=PubUtil.sql2JavaType(this._arrColumnType[fieldNo]);
		}
	...
	public String createContrData(ArrayList arrDimField, String strPerdFieldName, int iPerdType
			, String strDataFieldName, int iContrType) throws Exception{
		if (this._arrData.size() <= 0){
			return "";
		}
		...
		for (int i=0;i<arrDimField.size();i++){
			...
			iKeyDataType[i]=PubUtil.sql2JavaType(this._arrColumnType[dimFieldNo]);
		}
		...
		if (PubUtil.isEmptyString(minPerd)){
			return "";
		}
		...
		for (int iRow=0;iRow<this._arrData.size();iRow++){
			...
			if (PubUtil.isEmptyString(strPerd)){
				continue;
			}
			String strLastPerd=getLastPerd(minPerd, strPerd, iPerdType, iContrType);
			if (PubUtil.isEmptyString(strLastPerd)){
				continue;
			}
	...
	public Cds createGroupCds(ArrayList arrGroupField, ArrayList arrSumField, ArrayList arrFuncType
			, ArrayList arrDataType, String strMask, boolean bFromDetailData) throws Exception{
		Cds cdsSum=null;
		...
		//掩码
		if (false == PubUtil.isEmptyString(strMask)){
	...
	//从明细数据中汇总
	private Cds createGroupCdsFromDetail(ArrayList arrGroupField, ArrayList arrSumField, ArrayList arrFuncType
			, ArrayList arrDataType) throws Exception{
		//汇总字段数据类型
		...
		for (int i=0;i<arrSumField.size();i++){
			...
			int fieldNo=((Integer)_hmFieldName.get(strFieldName)).intValue();
			iValueDataType[i]=PubUtil.sql2JavaType(this._arrColumnType[fieldNo]);
		}
		...
		for (int i=0;i<arrSumField.size();i++){
			iDefDataType[i]=PubUtil.uq2JavaType((String)arrDataType.get(i));
		}
		...
		for (int i=0;i<arrSumField.size();i++){
			...
			//求平均：增加字段_SUM  _COUNT
			if (PubUtil.isEqualsString(strFuncType[i], PubConst.FUNC_TYPE_AVG)){   //平均数
				if (false == cdsSum._hmFieldName.containsKey(strFieldName + "_SUM"))
					cdsSum.appendField(strFieldName + "_SUM", java.sql.Types.DECIMAL, cdsSum._arrColumnLength[iFieldNo], cdsSum._arrColumnScale[iFieldNo], dDef);
				if (false == cdsSum._hmFieldName.containsKey(strFieldName + "_COUNT"))
					cdsSum.appendField(strFieldName + "_COUNT", java.sql.Types.DECIMAL, cdsSum._arrColumnLength[iFieldNo], cdsSum._arrColumnScale[iFieldNo], dDef);
			}else //计数，数据类型改为数值型
			if (PubUtil.isEqualsString(strFuncType[i], PubConst.FUNC_TYPE_COUNT)){    //计数
		...
		for (Iterator iter=key_Value.keySet().iterator();iter.hasNext();){
			...
			for (int i=0;i<arrSumField.size();i++){
				...
				if (PubUtil.isEmptyString(strFuncType[i])){
					//不处理
				}else if (PubConst.FUNC_TYPE_MAX.equals(strFuncType[i])
						|| PubConst.FUNC_TYPE_MIN.equals(strFuncType[i])
						|| PubConst.FUNC_TYPE_COUNT.equals(strFuncType[i])
						|| PubConst.FUNC_TYPE_SUM.equals(strFuncType[i])){  //最大值、最小值、计数、求和
	...
	//占比：对_SUM字段求和
	private Cds createGroupCdsFromSum(ArrayList arrGroupField, ArrayList arrSumFieldOri, ArrayList arrFuncTypeOri
			, ArrayList arrDataTypeOri) throws Exception{
		//比较数据库类型与定义类型是否一致
		//计数：改为求sum
		//计算平均值：***_SUM  、 ***_COUNT
		//占比的情况:仅计算***_SUM
		...
		for (int iOri=0;iOri<arrSumFieldOri.size();iOri++){
			...
			//求平均：增加字段_SUM  _COUNT
			if (PubUtil.isEqualsString(strFuncType, PubConst.FUNC_TYPE_AVG)){   //平均数
			...
			if (PubUtil.isEqualsString(strFuncType, PubConst.FUNC_TYPE_COUNT)){    //计数
		...
		for (int i=0;i<strFuncType.length;i++){
			if (PubUtil.isEqualsString(strFuncType[i], PubConst.FUNC_TYPE_AVG_FROM_SUM_DATA)){   //平均数
				String strFieldName=(String)arrSumField.get(i);
	...
	//计算汇总
	private void calc(TreeMap key_Value,ArrayList arrGroupField, ArrayList arrSumField, String[] strFuncType
			, int[] iValueDataType) throws Exception{
		//group by字段的数据类型
