...
public class FilterClause implements Comparable{
	...
	public FilterClause(String strFieldName,String strOperaor, Object objFilterValue) throws Exception{
		if (PubUtil.isEmptyString(strFieldName) || PubUtil.isEmptyString(strFieldName.trim())
				|| PubUtil.isEmptyString(strOperaor) || PubUtil.isEmptyString(strOperaor.trim())){
	...
	private FilterClause(String strLeftBracket,String strFieldName,String strOperaor, Object objFilterValue
		...
		if (false == (PubUtil.isEmptyString(strLeftBracket) || PubUtil.isEmptyString(strLeftBracket.trim()))){
			_iLeftBracketCount=strLeftBracket.trim().length();
		}
		if (false == (PubUtil.isEmptyString(strRightBracket) || PubUtil.isEmptyString(strRightBracket.trim()))){
		...
		if (PubUtil.isEqualsString(strAnd, "1")){
			_bAnd=false;
		}
	...
	public int compareTo(Object o) {
		...
		if (false == PubUtil.isEqualsString(this._strFieldName, newFilter._strFieldName)
				|| false == PubUtil.isEqualsString(this._strOperaor, newFilter._strOperaor)
		...
		if (this._objFilterValue instanceof String){
			if (newFilter._objFilterValue instanceof String[]){
				...
				if (PubUtil.isEqualsString((String)this._objFilterValue, (String)newFilter._objFilterValue)){
					return 0;
				}else{
					return -1;
				}
		...
		for (int i=0;i<me.length;i++){
			if (false == PubUtil.isEqualsString(me[i], other[i])){
				return -1;
			}
		}
