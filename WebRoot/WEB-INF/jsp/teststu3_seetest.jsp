				...
				<div class="span12">
					 <div class="widget-box">
				        <div class="widget-title"> <span class="icon"> <i class="icon-align-justify"></i> </span>
				          <h5>卷子：${test.testname}</h5>
				        </div>
				        <div class="widget-content nopadding updates">
				          <form action="${pageContext.request.contextPath}/servlet/StuScoreServlet" method="post" class="form-horizontal">
				             <input type="hidden" id="testid" name="testid" value="${test.testid}">
				             <c:forEach var="q" items="${test.qList}">
					            <div class="control-group">
						            <div class="new-update clearfix">
							            <div class="update-done">
								            <strong>题目：${q.q.qname}</strong> 
								            <span>${q.q.qcontent}</span>
								            <span>正确答案：${q.q.qright}</span>
								            <span>问题细节：${q.q.qdetail}</span>
								        	<span>上传人：${q.q.p.pname}</span>
								        </div>
							        </div>
					            </div>
				            </c:forEach>
