		...
		<div id="content-header">
			<div id="breadcrumb">
				<a href="${pageContext.request.contextPath}/servlet/IndexUIServlet"
					...
					class="current" class="current">学生评分</a>
			</div>
			<h1 style="font-family: Microsoft Yahei">学生评分</h1>
		</div>
							...
							<table id="question-table"  class="table table-bordered table-striped data-table  ">
							  ...
							  <tbody>
							    <c:forEach var="ss" items="${sslist}">
									<tr class="gradeX" >
										<input type="hidden" id="${ss.form.formid}" name="${ss.form.formid}" value="${ss.form.formid}" />
										<td>${ss.form.fname}</td>
										<td>${ss.p.pid}</td>
										<td>${ss.p.group}</td>
										<td>${ss.form.time}</td>
										<td>
											<c:if test="${ss.score==null||ss.score==0}">
												还没评论
											</c:if>
											<c:if test="${ss.score!=null&&ss.score!=0}">
												${ss.score}
										...
										<td style="text-align:center">
											<c:if test="${ss.score!=null&&ss.score!=0}">
												<a href="${pageContext.request.contextPath}/servlet/ScoreDetailServlet?formid=${ss.form.formid}&pid=${ss.p.pid}" class="btn btn-mini">评分表详情</a> 
											</c:if>
											<c:if test="${ss.score==null||ss.score==0}">
												<a href="${pageContext.request.contextPath}/servlet/GetStuScoreServlet?formid=${ss.form.formid}&pid=${ss.p.pid}" class="btn btn-primary btn-mini">评论</a> 
											</c:if>
